/*
  ...
  sends messages to it, reading from a KY-030 Microphone Sound Sensor sensor.
...
char topic[] = "randomNumberGenerator/sound";
// This should be a unique ID 
String clientID = "GW3869sound";
...
// once every interval, send a message:
  if (millis() - lastTimeSent > interval) {
    ...
    // get the sound readings from the sensor and store them in the variable ss
    float ss = analogRead(soundPin);

    // store the data object as a sting in the variable body
    String body = "{\"sound\": ss}";
    //replace the ss characters in the sting with the ss value as strings from the sensor
