...
/*
  ...
  and sends data from built in Arduion Nano 33 IoT gyroscope.
...
#include "secrets.h"
// library for the built in gyroscope
...
String clientID = "LSM6DS3GyroscopeClient-";
...
void setup() {
  ...
  // initalize the gyroscope sensor
  if (!IMU.begin()) {
...
// once every interval, send a message:
  if (millis() - lastTimeSent > interval) {
    ...
    // initialize variables and read gyroscope data
    float xx, yy, zz;
    IMU.readGyroscope(xx, yy, zz);

    String body = "{\"xx\": xx, \"yy\": yy,\"zz\": zz}";
   body.replace("xx", String(xx));
   body.replace("yy", String(yy));
   body.replace("zz", String(zz));
